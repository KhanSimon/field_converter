...
def build_grid() -> list[dict[str, Any]]:
    ...
    for patches in itertools.product(*GRID_DIMENSIONS):
        trial: dict[str, Any] = {}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Grid search for root TCN hyperparameters")
    parser.add_argument("--base-config", type=Path, default=DEFAULT_BASE_CONFIG)
    parser.add_argument("--search-name", type=str, default=DEFAULT_SEARCH_NAME)
    parser.add_argument("--start-index", type=int, default=0)
    parser.add_argument("--max-trials", type=int, default=None)
    parser.add_argument("--seed", type=int, default=1234)
    parser.add_argument("--shuffle", action="store_true", help="Shuffle grid order before slicing trials")
    ...
    parser.add_argument("--dry-run", action="store_true", help="Write grid configs without launching training")
...
def main() -> None:
    ...
    selected_grid = grid[args.start_index :]
    if args.max_trials is not None:
        selected_grid = selected_grid[: args.max_trials]
    if not selected_grid:
        raise ValueError(
            f"No grid trials selected. Grid has {len(grid)} trials, start-index={args.start_index}."
        )

    output_dir = Path(args.output_dir)
    output_dir_abs = output_dir if output_dir.is_absolute() else PROJECT_ROOT / output_dir
    search_dir = output_dir_abs / "grid_search" / args.search_name
    configs_dir = search_dir / "configs"
    logs_dir = search_dir / "logs"
    results_csv = search_dir / "grid_search_results.csv"
    results_jsonl = search_dir / "grid_search_results.jsonl"
        ...
        json.dumps(
            {
                ...
                "notes": "Generated by scripts/tcn/grid_search.py",
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    for offset, trial_hparams in enumerate(selected_grid):
        ...
        if args.dry_run:
            ...
            print(f"[trial {trial_idx:03d}/{len(grid) - 1:03d}] training {run_name}")
        ...
        row = {
            ...
            "best_root_error_mean_m": summary.get("best_root_error_mean_m"),
    ...
    print(f"Grid search records: {results_csv}")
