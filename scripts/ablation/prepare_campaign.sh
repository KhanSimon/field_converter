#!/bin/bash -l
#SBATCH --job-name=ablation_prepare
...
OVERWRITE=false
REGENERATE_ROOT_INIT=false

COMMAND=(python -m field_converter.ablation.prepare "$MANIFEST")
if [[ "$OVERWRITE" == true ]]; then
    COMMAND+=(--overwrite)
fi
if [[ "$REGENERATE_ROOT_INIT" == true ]]; then
    COMMAND+=(--regenerate-root-init)
fi
