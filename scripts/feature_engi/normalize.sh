...
PYTHONPATH=src python -c "from field_converter.data_preparation.normalize import Normalizer; Normalizer(in_features_dirname='features', out_features_dirname='features_normalized', pelvis_mode='hips_mean', min_bbox_size_px=10).run(train_n=65, valid_n=12, test_n=12, seed=12345, overwrite=True)"
